...
def set_contribution_flags(apps, schema_editor):
    ...
    donors = list(Donor.objects.annotate(count=Count('archive_photo_scanned')).filter(count__gte=1))
    for donor in donors:
        donor.is_scanner = True
    Donor.objects.bulk_update(donors, ['is_scanner'])

    donors = list(Donor.objects.annotate(count=Count('archive_photo_photographed')).filter(count__gte=1))
    for donor in donors:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('archive', '0068_auto_20230914_0628'),
    ]

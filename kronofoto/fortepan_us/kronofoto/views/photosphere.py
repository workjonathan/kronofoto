...
class ValidPhotoSphereView:
    ...
    def object(self) -> PhotoSphere:
        return get_object_or_404(PhotoSphere.objects.all(), pk=self.pk)
    ...
    def nearby_mainstreets(self) -> Iterable["WithAnnotations[MainStreetSet, ClosestAnnotation]"]:
        return MainStreetSet.objects.filter(Exists(self.all_nearby)).annotate(closest=Subquery(self.all_nearby.values("id")[:1])).order_by("name")
    ...
    @property
    #@icontract.ensure(lambda self, result: getattr(result, "context_data", None) is None)
